  ...
  <v-container class="mt-4">
    <v-row no-gutters>
      ...
      <v-flex align-self-center class="d-flex justify-end" v-if="loggedIn && article.author.id === userId">
        <v-btn fab small color="info" class="mx-1" depressed><v-icon small>fa-pen-to-square</v-icon></v-btn>
        <v-btn fab small color="error" depressed><v-icon small>fa-trash-can</v-icon></v-btn>
      </v-flex>
    </v-row>
    <v-row no-gutters>
      <v-col>
